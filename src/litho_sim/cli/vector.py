...
The vector effect: why hyper-NA lithography is polarised.
    ...
    litho-sim vector                  # results/vector_effect.png

(``scripts/demo_vector_imaging.py`` is the same command.)
...
def add_parser(subs) -> None:
    ap = subs.add_parser("vector", help="the polarisation effect at hyper-NA: law and images",
...
def run(args: argparse.Namespace) -> int:
    ...
    out = Path(args.output) / "vector_effect.png"
    ...
    curves: dict[str, list[float]] = {"te": [], "unpolarised": [], "tm": []}
    ...
    ax = fig.add_subplot(gs[0, 2])
    for pol, colour, lbl in (("te", TE_C, "TE"), ("unpolarised", UN_C, "unpolarised"),
                             ("tm", TM_C, "TM")):
        ax.plot(nas, curves[pol], "o-", color=colour, lw=2, ms=5, label=lbl)
    ax.set(xlabel="NA", ylabel="image contrast",
           title="Through the engine\n(x-dipole, 100 nm pitch, in resist)")
    ax.legend(fontsize=9)
    ax.grid(alpha=0.25)

    extent = (0.0, 128 * 4e-9 * 1e9, 0.0, 128 * 4e-9 * 1e9)
