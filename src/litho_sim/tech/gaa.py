...
Run ``litho-sim device gaa --figure results/gaa_steps.png`` to get one panel
per step with depositions in green and etched material in red
(``scripts/demo_gaa.py`` is the same command).
"""
...
EUV: dict[str, Any] = dict(wavelength=13.5e-9, NA=0.33, sigma_outer=0.8, sigma_inner=0.4,
...
def add_arguments(ap: argparse.ArgumentParser) -> None:
    """The flow's command-line flags, shared by the script and ``litho-sim device``."""
...
def run(args: argparse.Namespace) -> int:
    """Build, verify and report. Returns the process exit status."""
    if args.calibrate:
        run_calibration(
            build_gaa,
            levels=(("fin_litho", 96.0), ("gate_litho", args.gate_nm)),
            doses=(0.9, 1.05, 1.2, 1.35, 1.5),
            dose_keys=("fin_dose", "gate_dose"),
            sheets=args.sheets, gate_nm=args.gate_nm,
        )
        return 0

    rec = Recorder() if args.figure else None
    s, metrics = build_gaa(sheets=args.sheets, gate_nm=args.gate_nm, record=rec)
    c = report(s, s.shape_xy[1])
    if args.figure and rec is not None:
        render_steps(rec, s, args.figure)
...
def main(argv: list[str] | None = None) -> None:
    ...
    add_arguments(ap)
    sys.exit(run(ap.parse_args(argv)))
