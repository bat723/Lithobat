...
Run ``litho-sim device nfet`` for the flow and the structural checks
(``scripts/demo_nfet.py`` is the same command). Use
``scripts/render_devices_3d.py`` to see the finished device in 3-D.
"""
...
ARF_I: dict[str, Any] = dict(wavelength=193e-9, NA=1.35, n_immersion=1.44, sigma_outer=0.9,
             sigma_inner=0.6, source_type="annular", source_grid=11,
             normalisation="clear")
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.calibrate:
        run_calibration(
            build_nfet,
            levels=(("active_litho", 210.0), ("gate_litho", args.gate_nm)),
            doses=(0.9, 1.05, 1.2, 1.3, 1.45, 1.6),
            dose_keys=("active_dose", "gate_dose"),
            gate_nm=args.gate_nm,
        )
        return 0

    s, metrics = build_nfet(gate_nm=args.gate_nm)
